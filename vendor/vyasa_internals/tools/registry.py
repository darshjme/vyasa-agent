"""Central registry for all vyasa-agent tools.
...
Import chain (circular-import safe):
    tools/registry.py  (no imports from vyasa_internals.model_tools or tool files)
           ^
    tools/*.py  (import from vyasa_internals.tools.registry at module level)
           ^
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        """Return upstream-format tool schemas for the requested tool names.
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str:
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            if entry.is_async:
                from vyasa_internals.model_tools import _run_async
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from vyasa_internals.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
...
#
# Usage:
#   from vyasa_internals.tools.registry import registry, tool_error, tool_result
#
