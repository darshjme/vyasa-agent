...
Usage:
    from vyasa_internals.toolsets import get_toolset, resolve_toolset, get_all_toolsets
...
TOOLSETS = {
    ...
    "tts": {
        "description": "Text-to-speech: convert text to audio with Edge TTS (free), ElevenLabs, upstream vendor, or xAI",
    ...
    "code_execution": {
        "description": "Run Python scripts that call tools programmatically (reduces language model round trips)",
        "tools": ["execute_code"],
        "includes": []
    },
    ...
    # ==========================================================================
    # Full Vyasa toolsets (CLI + messaging platforms)
    #
    ...
    "vyasa-api-server": {
        "description": "upstream-compatible API server — full agent tools accessible via HTTP (no interactive UI tools like clarify or send_message)",
        "tools": [
            # Web
            "web_search", "web_extract",
            # Terminal + process management
    ...
    "vyasa-cli": {
        ...
        "tools": _VYASA_CORE_TOOLS,
        "includes": []
    },

    "vyasa-cron": {
        # Mirrors vyasa-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `vyasa tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant, rl) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as vyasa-cli; gated by `vyasa tools`",
        "tools": _VYASA_CORE_TOOLS,
        "includes": []
    },
    ...
    "vyasa-email": {
        "description": "Email bot toolset - interact with Vyasa via email (IMAP/SMTP)",
        "tools": _VYASA_CORE_TOOLS,
        "includes": []
    },
    ...
    "vyasa-sms": {
        "description": "SMS bot toolset - interact with Vyasa via SMS (Twilio)",
        "tools": _VYASA_CORE_TOOLS,
        "includes": []
    },
    ...
    "vyasa-gateway": {
        ...
        "tools": [],
        "includes": ["vyasa-telegram", "vyasa-discord", "vyasa-whatsapp", "vyasa-slack", "vyasa-signal", "vyasa-bluebubbles", "vyasa-homeassistant", "vyasa-email", "vyasa-sms", "vyasa-mattermost", "vyasa-matrix", "vyasa-dingtalk", "vyasa-feishu", "vyasa-wecom", "vyasa-wecom-callback", "vyasa-weixin", "vyasa-qqbot", "vyasa-webhook"]
    }
}
