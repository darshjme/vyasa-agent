...
from vyasa_internals.tools.registry import discover_builtin_tools, registry
...
def _get_tool_loop():
    ...
    *closes* a fresh loop every time) prevents "Event loop is closed"
    errors that occur when cached httpx/AsyncUpstreamClient clients attempt to
...
def _get_worker_loop():
    ...
    the coroutine, then *closes* the loop — but cached httpx/AsyncUpstreamClient
...
def _run_async(coro):
    ...
    loop so that cached async clients (httpx / AsyncUpstreamClient) remain bound
    ...
    # httpx/AsyncUpstreamClient clients bound to a live loop for the thread's
...
try:
    from vyasa_internals.tools.mcp_tool import discover_mcp_tools
    discover_mcp_tools()
...
try:
    from vyasa_internals.plugins import discover_plugins
    discover_plugins()
except Exception as e:
...
def get_tool_definitions(
    ...
    Returns:
        Filtered list of upstream-format tool definitions.
    """
    ...
    if enabled_toolsets is not None:
        ...
        from vyasa_internals.toolsets import get_all_toolsets
    ...
    if "execute_code" in available_tool_names:
        from vyasa_internals.tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
    ...
    if "discord_server" in available_tool_names:
        try:
            from vyasa_internals.tools.discord_tool import get_dynamic_schema
...
# so if something slips through, the language model sees a sensible message.
...
def coerce_tool_args(tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    language models frequently return numbers as strings (``"42"`` instead of ``42``)
...
def _coerce_json(value: str, expected_python_type: type):
    ...
    causes the language model to emit the array/object as a JSON string instead of a native
...
def handle_function_call(
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from vyasa_internals.plugins import get_pre_tool_call_block_message
            ...
            try:
                from vyasa_internals.plugins import invoke_hook
                invoke_hook(
                    "pre_tool_call",
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from vyasa_internals.tools.file_tools import notify_other_tool_call
